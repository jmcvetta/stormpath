...
//
// Note:  I think this function *might* produce a cryptographically secure
// nonce, but I am not an expert in cryptography.  You should think twice 
// before relying on it for important things.
...
func (s *Sauthc1Signer) SignRequest(req *http.Request, key ApiKey) error {
	now := time.Now()
	...
	*/
	...
	kDate := s.sign(dateStamp, kSecret, s.HashAlgo)
	kNonce := s.sign(nonce, kDate, s.HashAlgo)
	kSigning := s.sign(s.IdTerminator, kNonce, s.HashAlgo)
	//
	// Final signature
	//
	sig := s.sign(stringToSign, kSigning, s.HashAlgo)
	sigHex := hex.EncodeToString([]byte(sig))
	authHeader := s.AuthenticationScheme + " " +
		s.nameValuePair(s.Sauthc1Id, id) + ", " +
		s.nameValuePair(s.Sauthc1SignedHeaders, signedHeaders) + ", " +
		s.nameValuePair(s.Sauthc1Signature, sigHex)
	req.Headers[s.AuthorizationHeader] = authHeader
	return nil // Success!
}
...
func (s *Sauthc1Signer) nameValuePair(name, value string) string {
	return name + "=" + value
}
